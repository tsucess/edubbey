<?php
    ...
    if(!empty($fname) && !empty($lname) && !empty($email) && !empty($school) && !empty($country) && !empty($password)){
    ...
    if (in_array($img_ext, $extensions) === true) {
        ...
        if($avatar['size'] < 2000000){
            ...
            if (move_uploaded_file($tmp_name, "../img/users-img/".$avatar_name)) {
                ...
                $insert_query = "INSERT INTO users (firstname, lastname, email, avatar, school, country, userpassword) 
                VALUES ( '$fname', '$lname', '$email', '$avatar_name', '$school', '$country', '$hashed_password')";

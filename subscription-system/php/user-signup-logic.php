<?php
    ...
    if(!empty($fname) && !empty($lname) && !empty($email) && !empty($school) 
    ...
    if (in_array($img_ext, $extensions) === true) {
        ...
        if($avatar['size'] < 2000000){
            ...
            if (move_uploaded_file($tmp_name, "../img/users-img/".$avatar_name)) {
                ...
                $insert_query = "INSERT INTO users  (firstname, lastname, email, avatar, school, country, category_id, userpassword) 
                VALUES ( '$fname', '$lname', '$email', '$avatar_name', '$school', ' '$country', $category, '$hashed_password')";
                $insert_sql = mysqli_query($dbconnect, $insert_query);


                // also insert the new users id on result table
